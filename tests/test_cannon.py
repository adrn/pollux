...
class TestCannonTransformSequence:
    ...
    def test_cannon_with_lux_model(self):
        """Test using Cannon's TransformSequence with LVM."""
        rng = np.random.default_rng(42)
        n_stars = 50
        n_labels = 3
        n_pixels = 20
        ...
        # Create LVM with Cannon's transform
        ...
        # Predict using LVM with Cannon's coefficients
        predicted = model.predict_outputs({"flux": cannon_pars}, labels)
